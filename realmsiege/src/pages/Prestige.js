...
{branch:'Eternal Archive',nodes:'~25',key:'Bestiary bonuses apply run 1 without re-killing'},
...
export default function Prestige(){return(<div className="page"><div className="container section">
...
<tbody>{[['1st Clear','50','×1.0','50 pts'],['2nd Clear','50','×1.5','75 pts'],['3rd Clear','50','×2.0','100 pts'],['Nth Clear','50','×(1+0.5×N)','Scales indefinitely'],['Medium','×0.7 of Hard','same','35 pts base'],['Easy','×0.4 of Hard','same','20 pts base']].map(([r,b,m,tot])=>(
<tr key={r}><td>{r}</td><td>{b}</td><td>{m}</td><td style={{color:'var(--gold)'}}>{tot}</td></tr>))}</tbody>
</table>
</div>
<h2 style={{fontFamily:'Cinzel,serif',fontSize:'1.25rem',color:'#fff',marginBottom:'1rem'}}>Prestige Tree Branches</h2>
...
<span className="badge bm">{t.nodes} nodes</span>
</div>
<div style={{fontSize:'.84rem',color:'var(--muted)'}}>{t.key}</div>
</div>))}
</div>
<div style={{background:'rgba(200,150,12,0.05)',border:'1px solid var(--gold-dim)',borderRadius:4,padding:'1.5rem'}}>
<div style={{fontFamily:'JetBrains Mono,monospace',fontSize:'.58rem',letterSpacing:'.16em',textTransform:'uppercase',color:'var(--gold)',marginBottom:'.75rem'}}>Post-Prestige Mob Scaling</div>
<table className="data-table">
<thead><tr><th>Prestige</th><th>Mob HP Bonus</th><th>Mob DMG Bonus</th><th>New Features</th></tr></thead>
<tbody>{[['P1','+25%','+15%','Boss Altar, Shadow Runs, Lunar Stone tiles'],['P2','+50%','+30%','Champions from Wave 5, Rifts from Wave 1'],['P3','+80%','+50%','Void Crack tiles, Cursed waves permanent'],['P5','+120%','+75%','Faction Invasions every 10 waves from Level 5'],['P10','+200%','+120%','True Endless Mode unlocked past Level 25']].map(([p,h,d,f])=>(
<tr key={p}><td style={{color:'var(--gold)'}}>{p}</td><td style={{color:'#f87171'}}>{h}</td><td style={{color:'#f87171'}}>{d}</td><td style={{fontSize:'.8rem'}}>{f}</td></tr>))}</tbody>
</table>
</div>
</div></div>);}
